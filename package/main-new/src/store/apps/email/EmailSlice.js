...
export const EmailSlice = createSlice({
  ...
  reducers: {
    ...
    deleteEmail: (state, action) => {
      ...
      // If the deleted email was the one currently selected, select the next available one
      if (state.emailContent === idToDelete) {
        state.emailContent = state.emails.length > 0 ? state.emails[0].id : null;
      }
    },
    setVisibilityFilter: (state, action) => {
      state.currentFilter = action.payload;
